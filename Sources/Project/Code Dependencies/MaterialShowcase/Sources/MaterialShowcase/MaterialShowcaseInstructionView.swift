...
public class MaterialShowcaseInstructionView: UIView {
  ...
  public var primaryTextPositionX: CGFloat = 0.0
  public var primaryTextPositionY: CGFloat = 0.0
  public init() {
    // Create frame
  ...
  private func addSecondaryLabel() {
    ...
    private func addNextLabel() {
      ...
      if let font = secondaryTextFont {
        ...
        nextLabel.font = UIFont.systemFont(ofSize: secondaryTextSize)
      }
        nextLabel.text = secondaryText
        nextLabel.textColor = secondaryTextColor
        nextLabel.textAlignment = .center
        nextLabel.lineBreakMode = NSLineBreakMode.byWordWrapping
     //   nextLabel.text = secondaryText
        nextLabel.numberOfLines = 0
        
        nextLabel.frame = CGRect(x: primaryLabel.frame.width / 2,
                                    y: primaryLabel.frame.height + 16,
                                    width: 100,
                                    height: 40)
        //nextLabel.backgroundColor = UIColor.orange
        //MARK:- to change got it background color
        nextLabel.backgroundColor = UIColor.appColor
        nextLabel.layer.cornerRadius = 8
        nextLabel.layer.borderColor = UIColor.white.cgColor
        nextLabel.clipsToBounds = true
      
      //  nextLabel.sizeToFitHeight()
      addSubview(nextLabel)
      frame = CGRect(x: frame.minX, y: frame.minY, width: frame.width, height: primaryLabel.frame.height + nextLabel.frame.height)
    }
