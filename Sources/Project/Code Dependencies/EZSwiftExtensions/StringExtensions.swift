...
extension Data {
    var htmlString2AttributedString: NSAttributedString? {
        do {
...
extension String {
    ...
    func currencyInputFormatting() -> String {
        ...
        amountWithPrefix = regex.stringByReplacingMatches(in: amountWithPrefix, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, self.length), withTemplate: "")
    ...
    var html2AttributedString: NSAttributedString? {
        return data(using: .utf8)!.htmlString2AttributedString
    }
    var html2String: String {
...
//        return (firstMatch?.range.location != NSNotFound && firstMatch?.url?.scheme == "mailto")
    ...
    func getDateObject(dateFormate : String) -> Date{
        if self == ""{
            return Date()
        }
